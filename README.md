# CloudNav-abcd（修改版）
**本项目基于 https://github.com/sese972010/CloudNav- 进行修改，采用 Cloudflare KV 实现多端实时同步，适合个人使用！**

---
最新版本v1.8
- 1、**新增 Cloudflare KV 实时云同步功能**
  - 本地数据变更后自动同步到云端 (3秒 debounce)
  - 页面加载时自动检测云端数据并进行冲突检测
  - 多端同时修改时弹出冲突解决对话框，用户可选择保留本地或云端版本
- 2、新增同步状态指示器（右下角显示同步状态：已同步/同步中/待同步/同步失败）
- 3、支持 KV 版本历史快照备份（30天自动过期）
- 4、优化数据存储结构，增加版本号和设备ID用于冲突检测
...
- 3、调整鼠标悬浮和排序状态下的站点卡片背景颜色
- 4、新增链接编辑页面网址图标实时预览效果

版本v1.6
- 1、站外搜索网站的设置保存到服务器实现持久化
- 2、分类目录权限部分修改：`常用推荐`分类调整为可以移动位置
...
### ☁️ 数据同步
*   **Cloudflare KV 实时同步**: 利用边缘存储技术，公司、家里、手机三端数据秒级同步，自动冲突检测。
*   **本地优先**: 数据优先存储在浏览器 LocalStorage，确保离线可用，变更后自动同步到云端。
...
| AI 智能设置 | 云端同步 | 移动端适配 |
| :---: | :---: | :---: |
| ![AI Settings](screenshots/overview-light.svg) | ![Sync](screenshots/overview-dark.svg) | ![Mobile](screenshots/mobile-view.svg) |
| *一键批量生成描述* | *Cloudflare KV 多端同步* | *完美适配手机浏览器* |
...
5.  **绑定 KV**: 进入 Pages 项目设置 -> 绑定 (Bindings) -> 添加 KV 命名空间 -> 变量名填 `CLOUDNAV_KV`，值选择刚才创建的 `CLOUDNAV_DB`
6.  **部署**: 重新部署项目即可。
